...
##### Answer: Snowflake Architecture Explained (Image source: [Snowflake documentation](https://docs.snowflake.com/en/user-guide/intro-key-concepts))
...
##### Answer: Zero-copy cloning is a unique Snowflake feature that allows you to create a full copy of a database, schema, or table instantly — without duplicating the actual data.
...
To implement data masking, you:

* Create a masking policy using SQL that defines when to show the real data and when to mask it (for example, replace with stars or hide completely).

* Apply this masking policy to the sensitive column in your table.

* When users query the data, Snowflake checks their role. If the user has permission, they see the real data; otherwise, they see the masked version.

* This helps protect sensitive information dynamically without changing the actual stored data.

* Snowflake implements data masking using Dynamic Data Masking (DDM) with masking policies.

* You create a masking policy using CREATE MASKING POLICY that defines when to mask or show real data based on user role or context.

* For example, sensitive data like Social Security Numbers can be masked for most users but visible to privileged roles.

* After creating the policy, you apply it to a specific column using:
ALTER TABLE <table_name> MODIFY COLUMN <column_name> SET MASKING POLICY <policy_name>;

* When users query the data, Snowflake checks their role and dynamically shows either masked or real data.

* This method leverages Snowflake’s Role-Based Access Control (RBAC) for secure, flexible access.

* Importantly, the original data remains unchanged — only the query results are masked based on user permissions.
...
#### Q20:	How do you migrate a large on-premise DW to Snowflake?
...
________________________________________
#### Q36 :	How does caching work in Snowflake?
##### Answer: Caching in Snowflake
