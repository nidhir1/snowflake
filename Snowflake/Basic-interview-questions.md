...
________________________________________
2️⃣ Schema and Object Migration
...
________________________________________
3️⃣ Query & Data Access Optimization
•	**Avoid SELECT ***
...
________________________________________
5️⃣ Monitor & Analyze Costs Regularly
...
________________________________________
3️⃣ Key Management and Rotation
•	Snowflake employs a hierarchical key management system:
o	Each customer’s data is encrypted with unique data encryption keys (DEKs).
o	DEKs are encrypted with key encryption keys (KEKs).
o	KEKs are regularly rotated and securely stored.
•	Snowflake handles key lifecycle and rotation automatically, transparent to users.
________________________________________
4️⃣ Additional Security Features
...
________________________________________
1️⃣ ETL (Extract, Transform, Load) in Snowflake
•	Process:
...
________________________________________
2️⃣ ELT (Extract, Load, Transform) in Snowflake
•	Process:
...
•	Advantages:
o	Leverages Snowflake’s scalable compute for transformations.
o	Reduces data movement and latency.
o	Simplifies architecture by centralizing transformation logic.
•	Snowflake Features Supporting ELT:
o	Virtual Warehouses: Scalable compute clusters for transformation queries.
o	Streams & Tasks: Automate incremental data processing and scheduling.
o	Stored Procedures & UDFs: Encapsulate complex logic in Snowflake SQL or JavaScript.
o	Materialized Views: Precompute and cache expensive queries.
________________________________________
...
________________________________________
Setting Up Snowpipe (High-Level Steps)
1.	Create an external or internal stage pointing to your cloud storage bucket.
2.	Create a pipe with a COPY INTO statement defining how to load data.
3.	Configure event notifications on your cloud storage to trigger Snowpipe.
4.	Monitor Snowpipe via Snowflake’s views (SNOWPIPE_LOAD_HISTORY) to track ingestion status.
________________________________________
Example:
sql
CopyEdit
...
FILE_FORMAT = (TYPE = 'CSV' FIELD_OPTIONALLY_ENCLOSED_BY='"');
________________________________________
Benefits of Snowpipe for Continuous Ingestion
•	Eliminates the need for batch data loads.
•	Provides low-latency data availability for analytics.
•	Simplifies pipeline architecture with automated processing.
•	Minimizes manual scheduling or orchestration overhead.
________________________________________
💡 Interview Tip:
Highlight Snowpipe as Snowflake’s cloud-native, event-driven ingestion service that seamlessly integrates with cloud storage events to deliver near real-time data loading without managing infrastructure.

##### Ques 14 : What is the difference between shared-disk and shared-nothing architectures?
...
Difference Between Shared-Disk and Shared-Nothing Architectures
These two are common distributed database/storage architectures with different ways of managing data and compute resources.
________________________________________
Aspect	Shared-Disk Architecture	Shared-Nothing Architecture
Storage	All nodes access a shared storage system (disk or SAN).	Each node has its own local storage (no sharing).
Compute	Multiple nodes run compute but share the same disk.	Compute and storage are co-located on each node.
Data Management	Coordination needed to avoid conflicts on shared disk.	Data is partitioned across nodes; no overlap.
Scalability	Limited by storage system bottlenecks and contention.	Highly scalable by adding more nodes.
Fault Tolerance	Storage is centralized; if shared storage fails, system impacted.	More fault tolerant as each node operates independently.
Examples	Oracle RAC, IBM DB2 with shared disk setups.	Snowflake, Cassandra, Hadoop, Google Bigtable.
Concurrency Control	Needs complex locking mechanisms on shared storage.	Easier concurrency due to data partitioning.
Complexity	Can be complex due to synchronization needs.	Simpler coordination due to data locality.
________________________________________
Summary:
...
•	Includes:
o	User stages (one per user)
o	Table stages (one per table)
o	Named internal stages created explicitly by users.
...
________________________________________
Key Takeaways
•	Auto-Resume Feature: Helps reduce delays by automatically starting the warehouse when queries arrive, but there’s always a startup delay (usually a few seconds).
•	Auto-Suspend Feature: Saves cost but may increase latency for the first query after suspension due to warehouse startup.
...
________________________________________
Key Concepts:
1️⃣ Automatic Micro-Partitioning
...
3️⃣ Shared-Nothing Architecture
...
4️⃣ Clustering Keys (Optional Manual Control)
...
________________________________________
💡 Interview Tip:
Emphasize Snowflake’s automatic micro-partitioning combined with metadata pruning as a key innovation that simplifies data distribution and boosts query performance in a cloud-native, shared-nothing architecture.
...
2️⃣ Query Parsing and Optimization
